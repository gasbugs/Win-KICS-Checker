...
function Test-W78SecureChannelDataEncryptionSigning {
    ...
    try {
        ...
        $requireSignOrSeal = ($content | Select-String -Pattern "RequireSignOrSeal" | ForEach-Object { $_.ToString().Split('=')[1].Split(',')[1].Trim() }) -as [int]
        $sealSecureChannel = ($content | Select-String -Pattern "SealSecureChannel" | ForEach-Object { $_.ToString().Split('=')[1].Split(',')[1].Trim() }) -as [int]
        $signSecureChannel = ($content | Select-String -Pattern "SignSecureChannel" | ForEach-Object { $_.ToString().Split('=')[1].Split(',')[1].Trim() }) -as [int]
